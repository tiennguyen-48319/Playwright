...
export class CheckoutPage extends BasePage {
  ...
  readonly firstNameErrorMsg = this.page.locator(
    'li[data-id="billing_first_name"]'
  );
  readonly lastNameErrorMsg = this.page.locator(
    'li[data-id="billing_last_name"]'
  );
  readonly streetAddressErrorMsg = this.page.locator(
    'li[data-id="billing_address_1"]'
  );
  readonly cityErrorMsg = this.page.locator('li[data-id="billing_city"]');
  readonly zipCodeErrorMsg = this.page.locator(
  ...
  async selectCountry(countryName: string) {
    ...
    if (!current?.includes(countryName)) {
      ...
      await this.page
        .locator(`ul#select2-billing_country-results li:text("${countryName}")`)
        .click();
    }
  }
  ...
  async selectPaymentMethod(method: PaymentMethods) {
    const methodLocator = this.page.locator(
      `//li[label[contains(., '${method}')]]/input`
    );
