...
export class CartPage extends BasePage{
  ...
  async getAllProducts(): Promise<ProductInfo[]> {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const name = (await row.locator('.product-title').textContent())?.trim() ?? '';
      const type = (await row.locator('.products-page-cats').textContent())?.trim() ?? '';
      const star = (await row.locator('.star-rating').getAttribute('class')) ?? '';
